...
public:
	void ReadImage(Image<T> *inputImage, int conn8=1,
                 Image<unsigned char> *edgeMap = 0);

  void ExploitEdgeMap(Image<unsigned char> *edgeMap,
                      int x1, int y1, int x2, int y2, Edge& edge)
  {
    if (!edgeMap)
      return;
    if (edgeMap->Access(x1,y1) != 0 || edgeMap->Access(x2,y2) != 0)
      edge.w_ = 1e9;
  }
                                  ...
                                  Image<unsigned char> *edgeMap)
{
	...
	for (int y = 0; y < height_; y++)
	{
        ...
        ExploitEdgeMap(edgeMap, x, y, x+1, y, edges_[num]);
				num++;
			}
        ...
        ExploitEdgeMap(edgeMap, x, y, x, y+1, edges_[num]);
				num++;
			}
          ...
          ExploitEdgeMap(edgeMap, x, y, x+1, y+1, edges_[num]);
					num++;
				}
